...
import approveListJson from './approve-user-list.json';
...
const getApproveList = () => {
    const response = axios.get(mainURL + '/api/approve/info/' + user_id, {
        ...
        .then(function (response) {
            ...
            approveList.value = approveList.value.filter(item => item.status_type_id >= 5)
        })
            ...
            <div class="w-full   mx-auto ">
                <div class="relative overflow-hidden bg-white  dark:bg-gray-800 ">
                    <div
                        ...
                        <div>
                            <h5 class="mr-3 font-semibold dark:text-white text-3xl">
                                คำร้องที่ไม่ผ่านการอนุมัติจากผู้จัดการ</h5>
                            <p class="text-gray-500 ">รวมรายการเอกสารและโครงการที่ไม่ผ่านการอนุมัติจากผู้จัดการ</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <tbody v-if="approveList.length > 0">
                <tr v-for="item in approveList"
                    ...
                    <td class="pl-4 items-center justify-center">
                        <div v-if="item.isOwnerRead === 0" class="w-2 h-2 bg-red-500 border border-white rounded-full">
                        </div>
                    </td>
